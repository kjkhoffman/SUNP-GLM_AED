...
setwd('/Volumes/My Passport/GLM/Bootstrapped_TNTP') 
nuts<-read.csv("TN_TP_Inflow_conc_fracNP_boot_13Mar17.csv")
files<-list.files("/Volumes/My Passport/GLM/inflow files")
setwd('/Volumes/My Passport/GLM/inflow files') 

alldata <- do.call(rbind, lapply(files,
                                 function(x) cbind(read.csv(x),
                                                   name=strsplit(x,'_tot')[[1]][1])))
...
iALL <- iUNG %>% left_join(iBAL)%>%left_join(i505)%>% left_join(i790) %>% 
  left_join(i830)  %>% left_join(i788) %>% left_join(i510) %>% left_join(i540) %>% 
  left_join(i800) %>% left_join(i835) %>% left_join(i805)%>% left_join(i665)%>% left_join(i760)
...
iALL<-iALL%>%mutate(vwsTP=(TPung*FLOWung/totFLOW)+(TPBAL*FLOWBAL/totFLOW)+(TP505*FLOW505/totFLOW)+(TP790*FLOW790/totFLOW)+
                      ...
                      (TP540*FLOW540/totFLOW)+(TP800*FLOW800/totFLOW)+(TP835*FLOW835/totFLOW)+
...
iALL<-iALL%>%mutate(vwsTN=(TNung*FLOWung/totFLOW)+(TNBAL*FLOWBAL/totFLOW)+(TN505*FLOW505/totFLOW)+(TN790*FLOW790/totFLOW)+
                      ...
                      (TN540*FLOW540/totFLOW)+(TN800*FLOW800/totFLOW)+(TN835*FLOW835/totFLOW)+
...
iALL<-iALL%>%mutate(vwsTEMP=(TEMPung*FLOWung/totFLOW)+(TEMPBAL*FLOWBAL/totFLOW)+(TEMP505*FLOW505/totFLOW)+(TEMP790*FLOW790/totFLOW)+
                      ...
                      (TEMP540*FLOW540/totFLOW)+(TEMP800*FLOW800/totFLOW)+(TEMP835*FLOW835/totFLOW)+
...
oneINFLOW<-subset(oneINFLOW,select=c("time","FLOW","SALT","TEMP","OGM_doc","OGM_poc",
                                     ...
                                     "OGM_pop","PHS_frp_ads"))
write.csv(oneINFLOW,"oneInflow_18May17.csv", row.names = FALSE, quote = FALSE)
