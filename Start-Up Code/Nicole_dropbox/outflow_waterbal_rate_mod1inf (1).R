...
setwd('/Users/nicoleward/Dropbox/CNH-GLM/Sunapee/Experiments/20170320_Sunapee_NewInflowMess/waterbal') 
...
daily_P <-  metfile %>% 
  ...
  group_by(day=format(time, '%d'),
           year=format(time, '%Y'), #if you need year also
...
join.exp <- daily_precip %>% left_join(evap) %>% left_join(fill_stage)%>% left_join(inf790)
join.exp<-na.omit(join.exp)
...
join.exp<-join.exp %>% mutate(tot.inf = flow790)  

join.exp<-join.exp %>% mutate(tot.inf = tot.inf*86400/16071489) #inflow total in m per day
join.exp<-na.omit(join.exp)
join.exp<- join.exp %>% mutate(q.out = - d.stage + precip + tot.inf + evap) #original from this file

join.exp<- join.exp %>% mutate(q.in = d.volume - precip - interp_outflow - evap)##operations fro V@.R
join.exp<- join.exp %>% mutate(q.out = + d.stage + precip + tot.inf + evap)
join.exp<- join.exp %>% mutate(q.out = + d.stage + precip + tot.inf + evap)#!!!!try this
###check signs here?!?!!!!!!!
...
write.csv(inflow_waterbal,"inflow_bal_6inf_20Mar17.csv", row.names = FALSE, quote = FALSE)
write.csv(outflow_waterbal,"outflow_bal_6inf_20Mar17.csv", row.names = FALSE, quote = FALSE)
